...
namespace smartTechAuthenticator.Services.Customers
{
    public class CustomersService : ICustomersService
    {
        ...
        public async Task<PlansViewModel> GetPlans(Guid Id)
        {
            PlansViewModel data = new PlansViewModel();
            data = (from a in context.Plans
                    where a.Id == Id
        ...
        public (int TotalCount, int FilteredCount, dynamic Categorys) GetPlans(int skip, int take, string sortColumn, string sortColumnDir, string searchValue)
        {
            var category = context.Plans.Select(x => new
            {
                Id = x.Id,
               PlanName = x.PlanName,
                Price = x.Price,
            ...
            if (!(string.IsNullOrEmpty(sortColumn) && string.IsNullOrEmpty(sortColumnDir)))
            {
                ...
                {
                    case "PlanName":
                        category = sortColumnDir == "asc" ? category.OrderBy(x => x.PlanName) : category.OrderByDescending(x => x.PlanName);
                        break;
            ...
            if (!string.IsNullOrEmpty(searchValue))
            {
                category = category.Where(m => m.PlanName.Contains(searchValue));
            }
            int recordsTotal = category.Count();
            var data = category.Select(x => new
            {
        ...
        public async Task<ResponseModel> CreatePlans(PlansViewModel prod)
        {
            ResponseModel model = new ResponseModel();
            using (var transaction = context.Database.BeginTransaction())
            {
                try
                {
                    Plans pm = new Plans();
                    pm.Id = Guid.NewGuid();
                    pm.PlanName = prod.PlanName;
                    pm.Price = prod.Price;
                    pm.Validity = prod.Validity;
        ...
        public async Task<PlansViewModel> GetPlanDetail(string Id)
        {
            var CategoryInfo = (from a in context.Plans
                                where a.Id == new Guid(Id)
                                select new PlansViewModel
                                {
                                    PlanName = a.PlanName,
        ...
        public async Task<ResponseModel> UpdatePlanData(PlansViewModel prod)
        {
            ResponseModel model = new ResponseModel();
            using (var transaction = context.Database.BeginTransaction())
            {
                try
                {
                    var Info = context.Plans.FirstOrDefault(x => x.Id == prod.Id);
                    if (Info != null)
                    {
                        Info.PlanName = prod.PlanName;
                        Info.Price = prod.Price;
                        Info.Validity = prod.Validity;
                        Info.CreatedDate = DateTime.Now;
                        context.Plans.Update(Info);
                    }
                    await context.SaveChangesAsync();
                    await transaction.CommitAsync();
                    model.Status = Status.Success;
        ...
        public async Task<ResponseModel> DeletePlanDetails(PlansViewModel model1)
        {
            ResponseModel model = new ResponseModel();
            try
            {
                var Info = context.Plans.FirstOrDefault(x => x.Id == model1.Id);
                if (Info != null)
                {
                    var data = context.Plans.Find(Info.Id);
                    if (data != null)
                    {
                        context.Plans.Remove(data);
                    }
                    await context.SaveChangesAsync();
                    model.Status = Status.Success;
                }
